 ...
 */
return array(
	'title'      => __( 'Blog posts with right sidebar', 'sitetheme' ),
	...
	'content'    => '<!-- wp:group {"align":"full","style":{"spacing":{"padding":{"top":"var(--wp--custom--spacing--small, 1.25rem)","bottom":"var(--wp--custom--spacing--small, 1.25rem)"}}},"layout":{"inherit":true}} -->
					...
					<figure class="wp-block-image size-large"><img src="' . esc_url( get_template_directory_uri() ) . '/assets/images/flight-path-on-salmon.jpg" alt="' . esc_attr__( 'Illustration of a flying bird.', 'sitetheme' ) . '"/></figure>
					<!-- /wp:image -->
					...
					<h4 class="has-large-font-size"><em>' . esc_html__( 'Categories', 'sitetheme' ) . '</em></h4>
					<!-- /wp:heading -->

					<!-- wp:tag-cloud {"taxonomy":"category","showTagCounts":true} /-->

					<!-- wp:heading {"level":4,"fontSize":"large"} -->
					<h4 class="has-large-font-size"><em>' . esc_html__( 'Tags', 'sitetheme' ) . '</em></h4>
					<!-- /wp:heading -->
